...
textSize(12);
 ...
 if(currentMillis_2 - previousMillis_2 >= interval) {
    ...
    // if the clk is off turn it on and vice-versa:
   if (clkState_2 == arduino.HIGH){
      clkState_2 = arduino.LOW;
   }else{
      clkState_2 = arduino.HIGH;
   }
    // set the LED with the ledState of the variable:

    arduino.digitalWrite(clkPin_2, clkState_2);
  }
    }
   }
}
...
void controlEvent(ControlEvent theEvent) {
  ...
  if (theEvent.getName() == "com"){
    ...
    arduino.pinMode(clkPin_2, arduino.OUTPUT);
    arduino.pinMode(Logic_pin[0], arduino.OUTPUT);
    arduino.pinMode(Logic_pin[1], arduino.OUTPUT);
    arduino.pinMode(Logic_pin[2], arduino.OUTPUT);
    arduino.pinMode(Logic_pin[3], arduino.OUTPUT);
}else{
